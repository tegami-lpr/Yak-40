...
defineProperty("fuse_cd", globalPropertyf("sim/aircraft/bodies/acf_fuse_cd"))
...
local tro_table_11 = {{ 0.00,-1 },    -- STOP
                  {  0.10, 0.18 },	  -- IDLE     |  287 |      
                  {  0.40, 0.5797 },  -- 0.4 nom  |  998 |  6500 |
				  {  0.50, 0.7 },     -- 0.6 nom  | 1480 |  7770 |
				  {  0.55, 0.7514 },  -- 0.7 nom  | 1730 |  8270 |
                  {  0.60, 0.81954 }, -- 0.85 nom | 2094 |  8950 |
           	      {  0.74, 0.890 },   -- nominal  | 2470 |  9560 |
          	      {  1.00, 1.00 }}    -- takeoff  | 3307 | 10750 |
...
local truealt_table = {{ -50000, 0},
					   { 0.00, 0 },    -- on standard pressure zero level
					   {  9842, 0.000},
					   {  19685, 0.000},
					   {  26575, 0.000},
					   {  100000, 0.000}}

local cdalt_table = {{ -50000,  1},
					   {   0.00,  0.570000 },    
					   {   9842,  0.418000},
					   {  19685,  0.380000},
					   {  26575,  0.355000},
...
if passed > 0 then
	...
	local coef_alt = interpolate(truealt_table, alt)
	
    local fuse_cdalt = interpolate(cdalt_table, alt)
    set(fuse_cd, fuse_cdalt)

	-- set results for engine throttles
	-- engine 1
	local throttle_1 = 0
	
	if XP11 then throttle_1 = interpolate(tro_table_11, rud1_last) * alt_coef * t_coef + coef_alt
	...
	-- engine 2
	local throttle_2 = 0
	if XP11 then throttle_2 = interpolate(tro_table_11, rud2_last) * alt_coef * t_coef + coef_alt
	...
	-- engine 3
	local throttle_3 = 0
	if XP11 then throttle_3 = interpolate(tro_table_11, rud3_last) * alt_coef * t_coef + coef_alt
