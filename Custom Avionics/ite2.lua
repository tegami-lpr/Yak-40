...
local n2_table_11 = {{ -100, 0 },    -- bugs workaround
                  {  0, 0 },		-- stop
				  {  63.00, 53 },		-- IDLE           |  53%   |
				  {  79.80, 78.8},		-- 0.4 nominal    |  78,8% |
				  {  86.00, 85.9},		-- 0.6 nominal    |  86%   |
				  {  88.60, 88.5},		-- 0.7 nominal    |  88,5% |
				  {  91.80, 91.9 },		-- 0.85 nominal   |  92%   |
                  {  94.50, 95 },  	-- nominal            |  95%   |
           	      {  100, 101 },    -- takeoff            | 100,8% |
          	      {  10000, 110 }}    -- bugs workaround
...
function update()
	...
	if XP11 then
		left_angle = interpolate(n1_table_11, get(N1) * alt_coef1) * azs * 3.07 + 50 + coef_alt
		right_angle = interpolate(n2_table_11, get(N2) * alt_coef2) * azs * 3.07 + 50 + coef_alt	
	else
